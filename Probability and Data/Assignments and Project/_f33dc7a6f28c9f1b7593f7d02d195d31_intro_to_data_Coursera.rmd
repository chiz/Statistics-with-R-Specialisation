...
nycflights %>%
  group_by(month) %>%
  summarise(mean_month= mean(dep_delay)) %>%
  arrange(desc(mean_month))
...
nycflights%>%
  group_by(origin)%>%
  summarise(on_time= sum(dep_type=="on time")/n() ) %>%
...
nycflights%>%
  ...
  summarise(ot_proportion_dep_delayed=sum(arr_type=="on time")/n())
```